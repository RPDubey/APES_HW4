...
list_for_each(list,&current_proc->children){num_child++;}
...
struct pid* pid_struct = find_get_pid(mypid);//get the process structure from PID
proc_desc = pid_task(pid_struct,PIDTYPE_PID);//get task structure from Process structure
