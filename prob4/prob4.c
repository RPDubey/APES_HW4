...
data.proc_id_p = list_prev_entry(current,tasks)->pid;//current->pid;
data.vrt_p = list_prev_entry(current,tasks)->se.vruntime;//current->se.vruntime;
data.proc_id_n = list_next_entry(current,tasks)->pid;//current->pid;
data.vrt_n = list_next_entry(current,tasks)->se.vruntime;//current->se.vruntime;
...
printk(KERN_INFO "prev    PID & vruntime:%d  %llu\n",((data_struct*)&data)->proc_id_p,((data_struct*)&data)->vrt_p);
printk(KERN_INFO "current PID & vruntime:%d  %llu\n",((data_struct*)&data)->proc_id_c,((data_struct*)&data)->vrt_c);
printk(KERN_INFO "next    PID & vruntime:%d  %llu\n",((data_struct*)&data)->proc_id_n,((data_struct*)&data)->vrt_n);
ssleep(10);
}
ssleep(1);
}
